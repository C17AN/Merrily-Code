...
그러나 `v-show` 지시자는 이와 다르게 동작합니다.

---

## v-show 지시자

`v-if`가 조건부 렌더링 과정에서 DOM 트리를 직접 조작했다면 `v-show`는 조건이 참일 때는 정상적으로 블록을 렌더링하고, 조건이 거짓일 때는 해당 블록에 `display: none` 인라인 스타일을 적용합니다.
